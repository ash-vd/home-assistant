class SliderEntityRow extends Polymer.Element {
  ...
  setConfig(config) {
    const CONTROLLERS = {
      light: {
        ...
        supported: {
          slider: stateObj => {
            if (stateObj.state === "off" && this._config.hide_when_off)
              return false;
            if ("brightness" in stateObj.attributes) return true;
            if (
              "supported_features" in stateObj.attributes &&
      ...
      media_player: {
        ...
        string: (stateObj, l18n) => {
          ...
          return !!stateObj.attributes.volume_level
            ? `${this.controller.get(stateObj)} %`
            : l18n["state.media_player.off"];
        },
        min: () => 0,
        max: () => 100,
        step: () => 5
      ...
      cover: {
        ...
        supported: {
          slider: stateObj => {
            if (stateObj.attributes.hasOwnProperty("current_position"))
              return true;
            if (
              stateObj.attributes.hasOwnProperty("supported_features") &&
              stateObj.attributes.supported_features & 4
            )
              return true;
            return false;
          },
      ...
      fan: {
        ...
        supported: {
          slider: stateObj => {
            return (
              !(stateObj.state === "off" && this._config.hide_when_off) &&
      ...
      input_select: {
        ...
        supported: {
          slider: stateObj => {
            return (
              stateObj.attributes.hasOwnProperty("options") &&
              stateObj.attributes.options.length > 1
            );
          },
          toggle: () => false
        },
    ...
    if (this._hass && this._config) {
      this.stateObj =
        this._config.entity in this._hass.states
  ...
  _isRTL() {
    ...
    const retval =
      this._hass.translationMetadata.translations[lang].isRTL || false;
    return retval;
  }
